...
DECLARE _sql text;
_during tstzrange;
BEGIN --if start or end is null ,use  infnity
_during := tstzrange(
...
DECLARE _sql text;
_offset bigint;
BEGIN -- if page_size is not between 1 and 100, set it to 10
IF page_size < 1
OR page_size > 100 THEN page_size := 10;
END IF;
-- if cursor is null or less than 0, set it to 0 if is_desc is false, or to max(bigint) if is_desc is true
IF cursor IS NULL
OR cursor < 0 THEN IF is_desc THEN cursor := max(bigint);
ELSE cursor := 0;
END IF;
END IF;
-- format the query based on parameters
